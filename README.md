...
* [Extracting best BPP trees](#Extracting-best-BPP-trees)
* [Cropping FASTA alignments](#Cropping-FASTA-alignments)
* [Scanning nonsynonymous-synonymous substitutions](#Scanning-nonsynonymous-synonymous-substitutions)
* [Extracting unique orthologs](#Extracting-unique-orthologs)
...
While [FASTA](https://www.bioinformatics.nl/tools/crab_fasta.html) alignments of big portion of a genome (e.g. full gene) have many purposes, some comparative genomic analysis required to crop these alignment into smaller ones (e.g. one exon or one CDS). The script `FastaCrop.py` crops nucleotide sequence alignment in FASTA format (`.fa`) according to specific coordinates described in a [BED](https://genome.ucsc.edu/FAQ/FAQformat.html#format1) file. The results will be stored into one output file with the suffix `_CDS.fa`.
...
___
## Scanning nonsynonymous-synonymous substitutions
The script `MutScan.py` scans coding nucleotide sequence alignments (in [FASTA](https://www.bioinformatics.nl/tools/crab_fasta.html) format; `.fa`), compares the nucleotide and amino-acid sequences between two aligned sets of sequences, and identifies the nonsynonymous and synonymous substitutions. The script also checks the open reading frame (ORF), and translates the nucleotide sequences into amino-acid sequences using the standard codon table (i.e. the preferential conversion table for *Anopheles gambiae* according to [NCBI](https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?mode=info&id=7165) and which correspond to the default codon table implemented in the [translate function of BioPython](https://biopython.org/docs/1.75/api/Bio.Seq.html)). For each alignment, the nonsynonymous and synonymous substitutions will be stored in a file (`file_MutScan.out`), and the nucleotide and amino-acid sequences will be exported into new FASTA files (`file_ORF.fa` and `file_ORF_AA.fa`, respectively).
...
| Name | Description |
| :--: | ----------- | 
| `ortho` | Excel file describing the *Drosophila melanogaster* orthologs (generated with BioMart) |

#### Notes
To retrieve the *D. melanogaster* orthologs of a list of *An. gambiae* genes, follow the instructions below: (1) Open [BioMart](https://bmcgenomics.biomedcentral.com/articles/10.1186/1471-2164-10-22) available on [VectorBase](https://biomart.vectorbase.org/biomart/martview/abe9742c0c6ad789305c2f72967d9fb7); (2) Choose the first dataset: `VectorBase genes`; (3) Choose the second dataset: `Anopheles gambiae genes (AgamP4.12)`; (4) Open the `Gene` tab from the `Filters` navigation bar, and import the gene list as the `Input external references ID list`; (5) Select the `homologues` option in the `Attributes` navigation bar; (6) Open the `GENE` tab from the `Attributes` navigation bar, and pick the information to be included in the output (e.g. Gene stable ID, chromosome name, start, stop...); (7) Open the `ORTHOLOGUES` tab from the `Attributes` navigation bar, select `Drosophila melanogaster Orthologues`, and pick the information to be included in the output (e.g. Gene stable ID, chromosome name, start, stop...); (8) Click on `Results` from the menu bar to preview the results of the query; (9) Export the unique results in Excel format by selecting the corresponding parameter.
To run the script `BioMartOrtho.py`, ensure that you are using Python v.2.7, and have installed the following dependencies: os, panda, and argparse.
