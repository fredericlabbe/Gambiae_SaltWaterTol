...
The script `ExtrBestTree.py` extracts the best tree from the results generated by the A01 analysis of [BPP](https://academic.oup.com/cz/article/61/5/854/1821090) (.out), and add them into one output file called `bestrees.out`. This list of the trees can then be used to build a consensus phylogenetic tree (e.g. using [ASTRAL-III](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-018-2129-y)).

#### Example command
`python ExtrBestTree.py`

#### Notes
The script ExtrBestTree.py needs to be run from the directory containing the results generated by the A01 analysis of BPP.
