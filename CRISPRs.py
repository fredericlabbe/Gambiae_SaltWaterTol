...
Checking the occurrence of overlapping forward and reverse CRISPR target sites using a reference genome in the FASTA format.
...
For each forward CRISPR target sites, this script exports its coordinates (i.e. chromosome, start and end), its sequence (e.g. ATGCGCCACACTTGACACTGG), and its strand (i.e. for).
For each reverse CRISPR target sites, this script exports its coordinates (i.e. chromosome, start and end), its reverse complement sequence (e.g. from CCAGTGTCAAGTGTGGCGCAT to ATGCGCCACACTTGACACTGG), and its strand (i.e. rev).
Note: this script will not check the occurrence CRISPR target sites within soft-masked parts of the reference genome (i.e. in lower-case, e.g. repetitive elements and low-complexity sequences).
usage: python CRISPRs.py --size 18 --ref file.fasta --out results.out
"""
...
pattern = '(?=([ATCG]{'+args.size+'}[ATCGN][G]{2}|[C]{2}[ATCGN][ATCG]{'+args.size+'}))'
